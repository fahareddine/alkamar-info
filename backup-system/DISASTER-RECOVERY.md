# Guide de Disaster Recovery — Alkamar Info
...
git clone https://github.com/fahareddine/alkamar-info.git
cd alkamar-info

# 2. Récupérer les secrets depuis Google Drive ou B2
#    Télécharger alkamar-secrets-TIMESTAMP.tar.gz.gpg
#    Puis déchiffrer :
echo "$GPG_PASSPHRASE" | gpg --batch --passphrase-fd 0 \
  --decrypt alkamar-secrets-TIMESTAMP.tar.gz.gpg | tar xz
...
#    Aller sur drive.google.com → Backups/AlkamarInfo/code/
#    Télécharger le zip le plus récent : alkamar-code-TIMESTAMP.zip

# Ou via rclone (si installé) :
rclone copy "gdrive:Backups/AlkamarInfo/code/" ./recovery/

# 2. Extraire
unzip alkamar-code-TIMESTAMP.zip -d alkamar-info-recovered/
cd alkamar-info-recovered/

# 3. Réinitialiser git (si nécessaire)
git init
git remote add origin https://github.com/NEW_ACCOUNT/alkamar-info.git
git add -A
git commit -m "feat: restauration depuis backup"
git push -u origin main

# 4. Récupérer les secrets (depuis Drive ou B2)
rclone copy "gdrive:Backups/AlkamarInfo/secrets/" ./secrets/
echo "$GPG_PASSPHRASE" | gpg --batch --passphrase-fd 0 \
  --decrypt secrets/alkamar-secrets-TIMESTAMP.tar.gz.gpg | tar xz
cp env.local .env.local
...
unzip recovery-b2/alkamar-code-TIMESTAMP.zip -d alkamar-restored/

# 4. Télécharger les secrets
rclone copy b2:info-experts-alkamar-backups/secrets/ ./recovery-secrets/
echo "$GPG_PASSPHRASE" | gpg --batch --passphrase-fd 0 \
  --decrypt recovery-secrets/alkamar-secrets-TIMESTAMP.tar.gz.gpg | tar xz

# 5. Télécharger la DB Supabase
rclone copy b2:info-experts-alkamar-backups/supabase/ ./recovery-supabase/
echo "$GPG_PASSPHRASE" | gpg --batch --passphrase-fd 0 \
  --decrypt recovery-supabase/alkamar-supabase-TIMESTAMP.tar.gz.gpg | tar xz
...
rclone copy "gdrive:Backups/AlkamarInfo/supabase/" ./recovery-supa/
# ou: b2:info-experts-alkamar-backups/supabase/

# 2. Déchiffrer
echo "$GPG_PASSPHRASE" | gpg --batch --passphrase-fd 0 \
  --decrypt recovery-supa/alkamar-supabase-TIMESTAMP.tar.gz.gpg | tar xz
...
echo "VOTRE_PASSPHRASE_GPG" | gpg --batch --passphrase-fd 0 \
  --decrypt alkamar-code-TIMESTAMP.zip.gpg > alkamar-code-TIMESTAMP.zip
# Ou si non chiffré :
unzip alkamar-code-TIMESTAMP.zip -d alkamar-info/
cd alkamar-info/
```
...
echo "VOTRE_PASSPHRASE_GPG" | gpg --batch --passphrase-fd 0 \
  --decrypt alkamar-supabase-TIMESTAMP.tar.gz.gpg | tar xz
...
vercel login
vercel link --name alkamar-info-restored
vercel --prod
...
wrangler pages deploy . --project-name=alkamar-info
```
...
gh release download "pre-deploy-COMMIT" \
  --repo fahareddine/info-experts-alkamar-backups \
  --dir ./rollback/

echo "$GPG_PASSPHRASE" | gpg --batch --passphrase-fd 0 \
  --decrypt rollback/alkamar-supabase-TIMESTAMP.tar.gz.gpg | tar xz -C ./rollback/
