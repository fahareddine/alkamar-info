#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# restore.sh — Restauration interactive Alkamar Info
#
...
#       BACKUP_REPO            : fahareddine/info-experts-alkamar-backups
#       GDRIVE_FOLDER          : Backups/AlkamarInfo
...
GDRIVE_FOLDER="${GDRIVE_FOLDER:-Backups/AlkamarInfo}"
...
echo ""
echo "╔══════════════════════════════════════════════════════════╗"
echo "║      OUTIL DE RESTAURATION — ALKAMAR INFO               ║"
...
mkdir -p "${RESTORE_DIR}"
mkdir -p "${LOG_DIR}"
WORK_DIR="$(mktemp -d /tmp/alkamar-restore-XXXXXX)"
...
download_archives() {
  case "$FROM_SOURCE" in
    local)
      CODE_ARCHIVE=$(ls "${BACKUP_DIR}/alkamar-code-${SNAPSHOT_TS}*.tar.gz" 2>/dev/null | head -1 || echo "")
      SUPA_ARCHIVE=$(ls "${BACKUP_DIR}/alkamar-supabase-${SNAPSHOT_TS}*.gpg" 2>/dev/null | head -1 || echo "")
      SECRETS_ARCHIVE=$(ls "${BACKUP_DIR}/alkamar-secrets-${SNAPSHOT_TS}*.gpg" 2>/dev/null | head -1 || echo "")
      ;;
    github)
      ...
      gh release download "supabase-${SNAPSHOT_TS}" \
        --repo "${BACKUP_REPO}" \
        --dir "${WORK_DIR}/gh-download" 2>> "${LOG_FILE}" || true
      CODE_ARCHIVE=$(ls "${WORK_DIR}/gh-download/alkamar-code-${SNAPSHOT_TS}"*.tar.gz 2>/dev/null | head -1 || echo "")
      SUPA_ARCHIVE=$(ls "${WORK_DIR}/gh-download/alkamar-supabase-${SNAPSHOT_TS}"*.gpg 2>/dev/null | head -1 || echo "")
      ;;
    gdrive)
      ...
      rclone copy "gdrive:${GDRIVE_FOLDER}/secrets/" "${WORK_DIR}/gdrive-download/" \
        --include "*${SNAPSHOT_TS}*" 2>> "${LOG_FILE}" || true
      CODE_ARCHIVE=$(ls "${WORK_DIR}/gdrive-download/alkamar-code-${SNAPSHOT_TS}"*.tar.gz 2>/dev/null | head -1 || echo "")
      SUPA_ARCHIVE=$(ls "${WORK_DIR}/gdrive-download/alkamar-supabase-${SNAPSHOT_TS}"*.gpg 2>/dev/null | head -1 || echo "")
      SECRETS_ARCHIVE=$(ls "${WORK_DIR}/gdrive-download/alkamar-secrets-${SNAPSHOT_TS}"*.gpg 2>/dev/null | head -1 || echo "")
      ;;
    b2)
      ...
      rclone copy "b2:${B2_BUCKET}/secrets/" "${WORK_DIR}/b2-download/" \
        --include "*${SNAPSHOT_TS}*" 2>> "${LOG_FILE}" || true
      CODE_ARCHIVE=$(ls "${WORK_DIR}/b2-download/alkamar-code-${SNAPSHOT_TS}"*.tar.gz 2>/dev/null | head -1 || echo "")
      SUPA_ARCHIVE=$(ls "${WORK_DIR}/b2-download/alkamar-supabase-${SNAPSHOT_TS}"*.gpg 2>/dev/null | head -1 || echo "")
      SECRETS_ARCHIVE=$(ls "${WORK_DIR}/b2-download/alkamar-secrets-${SNAPSHOT_TS}"*.gpg 2>/dev/null | head -1 || echo "")
      ;;
  esac
}
