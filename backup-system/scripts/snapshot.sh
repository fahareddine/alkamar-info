#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# snapshot.sh — Point de restauration complet Alkamar Info
#
...
if [[ ! -f "${INDEX_FILE}" ]]; then
  cat > "${INDEX_FILE}" <<'EOF'
# Index des snapshots — Alkamar Info
...
GITHUB_URL="https://github.com/${BACKUP_REPO:-fahareddine/info-experts-alkamar-backups}/releases/tag/code-${TIMESTAMP}"
GDRIVE_URL="gdrive:${GDRIVE_FOLDER:-Backups/AlkamarInfo}/"
