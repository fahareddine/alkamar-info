#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# verify.sh — Vérification d'intégrité des sauvegardes Alkamar Info
#
...
WORK_DIR="$(mktemp -d /tmp/alkamar-verify-XXXXXX)"

log "═══════════════════════════════════════════════════════════"
log "  Vérification intégrité — Alkamar Info"
...
verify_local() {
  ...
  LATEST_CODE=$(ls "${BACKUP_DIR}/alkamar-code-"*.zip 2>/dev/null | sort -r | head -1 || echo "")
  LATEST_SUPA=$(ls "${BACKUP_DIR}/alkamar-supabase-"*.gpg 2>/dev/null | sort -r | head -1 || echo "")
  LATEST_SECRETS=$(ls "${BACKUP_DIR}/alkamar-secrets-"*.gpg 2>/dev/null | sort -r | head -1 || echo "")
...
cat > "${REPORT_FILE}" <<EOF
# Rapport de vérification intégrité — Alkamar Info
