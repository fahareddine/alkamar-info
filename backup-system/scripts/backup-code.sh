#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# backup-code.sh — Sauvegarde du code source Alkamar Info (multi-cloud)
#
...
# Prérequis :
#   - git (installé)
#   - gh CLI (authentifié, repo cible créé: github.com/fahareddine/info-experts-alkamar-backups)
#   - rclone (configuré avec remotes 'gdrive' et 'b2')
#   - sha256sum ou shasum (standard Linux/macOS)
#   - Variables d'environnement :
#       BACKUP_REPO      : Repo GitHub cible (ex: fahareddine/info-experts-alkamar-backups)
#       GDRIVE_FOLDER    : Dossier Drive cible (ex: Backups/AlkamarInfo)
...
ARCHIVE_NAME="alkamar-code-${TIMESTAMP}${TAG_LABEL:+-${TAG_LABEL}}.tar.gz"
...
log "═══════════════════════════════════════════════════════════"
log "  Backup Code Source — Alkamar Info"
...
if [[ "$DRY_RUN" == "false" && "$SUCCESS_COUNT" -ge 2 ]]; then
  ...
  find "${BACKUP_DIR}" -name "alkamar-code-*.zip" -mtime +7 -delete 2>/dev/null || true
fi
