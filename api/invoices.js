...
module.exports = async function handler(req, res) {
  ...
  // Route fusionnée : /api/invoices/:id → /api/invoices?id=:id
  if (req.query.id) {
    ...
    const { data, error } = await supabase
      .from('invoices')
      .select(`*, orders(id, status, payment_method, payment_status, notes, coupon_code, discount_eur, order_items(quantity, unit_price_eur, unit_price_kmf, product_snapshot)), customers(id, name, email, phone, address, city, island)`)
      .eq('id', req.query.id)
      .single();
    if (error) {
