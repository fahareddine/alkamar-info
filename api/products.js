...
module.exports = async function handler(req, res) {
  ...
  // Route fusionnée : /api/products/:id/images → /api/products?_route=product_images&_id=:id
  if (req.query._route === 'product_images') {
      ...
      if (action === 'process') {
        ...
        try {
          const result = await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error('TIMEOUT')), 85000)),
          ]);
          return res.status(200).json(result);
        } catch (err) {
  ...
  // Route fusionnée : /api/products/:id → /api/products?_route=product_id&_id=:id
