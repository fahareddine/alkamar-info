...
async function sendEmail({ to, subject, html, text }) {
  ...
  try {
    const result = await resendRequest({
      from: EMAIL_FROM, reply_to: EMAIL_REPLY_TO,
      to: [recipient], subject: finalSubj,
      html, text: text || stripHtml(html),
    }, apiKey);
...
async function logEmail({ eventType, recipientEmail, subject, status, providerId, orderId, contactId, errorMsg }) {
  try {
    ...
    await supabase.from('email_logs').insert({
      event_type: eventType, recipient_email: recipientEmail, subject,
      status: status || 'sent', provider_message_id: providerId || null,
      related_order_id: orderId || null, related_contact_id: contactId || null,
      error_message: errorMsg || null,
    });
  } catch (e) { console.warn('[email] logEmail failed:', e.message); }
}
...
function nextSteps(steps) {
  const rows = steps.map((s, i) =>
    `<tr><td valign="top" style="padding:4px 0"><span style="display:inline-block;width:20px;height:20px;background:#1e3a8a;color:#fff;border-radius:50%;font-size:10px;font-weight:700;text-align:center;line-height:20px;margin-right:8px">${i + 1}</span><span style="font-size:13px;color:#374151">${esc(s)}</span></td></tr>`
  ).join('');
...
<p style="margin:0 0 20px;font-size:14px;color:#374151;line-height:1.7">Merci de nous avoir contact&eacute;s. Notre &eacute;quipe vous r&eacute;pondra dans les <strong>meilleurs d&eacute;lais</strong>, g&eacute;n&eacute;ralement sous 24&ndash;48 heures.</p>
...
<div style="background:#fef3c7;border:1px solid #f59e0b;border-radius:8px;padding:16px 20px;margin:16px 0">
  ...
  const steps = [
    ...
    order.payment_method === 'mobile_money' ? `Effectuez le paiement Mobile Money avec la référence #${orderNum}.` : order.payment_method === 'stripe' ? 'Votre paiement par carte est en attente de confirmation.' : 'Un conseiller vous contactera pour confirmer votre commande.',
    order.delivery_method === 'home_delivery' ? 'Vous serez contacté pour organiser la livraison.' : 'Rendez-vous en boutique à Moroni pour récupérer votre commande.',
  ];
  const content = `
<p style="margin:0 0 4px;font-size:13px;color:#6b7280;font-weight:600">Bonjour ${esc(name)},</p>
...
<tr><td style="font-size:13px;color:#6b7280;font-weight:600;padding:8px 0;border-top:2px solid #e5e7eb">Sous-total</td><td style="text-align:right;font-size:13px;color:#1f2937;font-weight:600;padding:8px 0;border-top:2px solid #e5e7eb">${formatPrice(order.subtotal_eur || order.total_eur)} &euro;</td></tr>
${order.delivery_fee > 0 ? `<tr><td style="font-size:13px;color:#6b7280;padding:4px 0">Livraison</td><td style="text-align:right;font-size:13px;color:#1f2937;padding:4px 0">${formatPrice(order.delivery_fee)} &euro;</td></tr>` : ''}
<tr><td style="font-size:15px;font-weight:800;color:#0f172a;padding:10px 0;border-top:2px solid #0f172a">Total</td><td style="text-align:right;font-size:15px;font-weight:800;color:#dc2626;padding:10px 0;border-top:2px solid #0f172a;white-space:nowrap">${formatPrice(order.total_eur)} &euro; <span style="font-size:11px;color:#6b7280;font-weight:400">(&asymp;&nbsp;${Math.round((Number(order.total_eur) || 0) * 491).toLocaleString('fr-FR')}&nbsp;KMF)</span></td></tr>
</table>
${mmBlock}
${nextSteps(steps)}
<div style="background:#f8fafc;border:1px solid #e5e7eb;border-radius:8px;padding:16px 20px;margin:20px 0">
