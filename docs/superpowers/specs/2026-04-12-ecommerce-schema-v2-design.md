...
CREATE TABLE stock_movements (
  ...
  quantity       INT NOT NULL,                -- positif ou négatif selon le sens du mouvement
...
BEGIN
  -- quantity est signé : positif = entrée, négatif = sortie
  -- Pour 'in' et 'return' : quantity doit être > 0
  -- Pour 'out' : quantity doit être > 0 (on soustrait)
  -- Pour 'adjustment' : quantity peut être positif ou négatif
  IF NEW.type IN ('in', 'return') THEN
    UPDATE products SET stock = stock + ABS(NEW.quantity) WHERE id = NEW.product_id;
  ELSIF NEW.type = 'out' THEN
    UPDATE products SET stock = stock - ABS(NEW.quantity) WHERE id = NEW.product_id;
  ELSE -- adjustment : quantity signé appliqué directement
